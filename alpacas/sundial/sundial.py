...
class Sundial:
    def __init__(self, latitude=45, longitude=0, orientation="horizontal"):
        ...
        if isinstance(orientation, str) and orientation in [
            "horizontal",
            "vertical",
            "equatorial_upper_side",
            "equatorial_lower_side",
            "polar",
        ]:
            ...
            raise Exception(
                "orientation must be either one out of 'horizontal', 'vertical', 'equatorial_upper_side', 'equatorial_lower_side', 'polar' or a list of length 2 with the angles of the dial."
            )
    ...
    def _nodus(self, tau, delta):
        if self.orientation == "horizontal":
            ...
            return bf.nodus_arbitrary_orientation(
                tau, delta, self.phi, self.orientation[0], self.orientation[1]
            )
    ...
    def _gnomon(self, tau):
        if self.orientation == "horizontal":
            ...
            raise Exception(
                "The polar gnomon is parallel to the dial of a polar sundial."
            )
        else:
            return bf.gnomon_arbitrary_orientation(
                tau, self.phi, self.orientation[0], self.orientation[1]
            )
    ...
    def _origin_polar_gnomon(self):
        if self.orientation == "horizontal":
            ...
            self.orientation == "equatorial_upper_side"
            or self.orientation == "equatorial_lower_side"
        ):
            opg = np.array([0, 0])
        elif self.orientation == "polar":
            raise Exception(
            ...
            opg = bf.position_gnomon_arbitrary_orientation(
                self.phi, self.orientation[0], self.orientation[1]
            )
        return opg
    ...
    def _max_decs(self, f):
        ...
        decs = np.arange(
            -const.ECLIPTIC / 180 * np.pi,
            const.ECLIPTIC / 180 * np.pi + self.eps,
            self.eps,
        )
        if not self.nh:
            decs = np.flip(decs)
    ...
    def add_date_line(
        self, date, style=None, label=None, labelstyle=None, rendering=0.01
    ):
        """
        ...
        if date == "equinox":
            ...
            raise Exception(
                "Date must be either 'equinox', 'summer_solstice', 'winter_solstice' or a date specified as [month, day]"
            )

        taumax = bf.sunset(delta, self.phi)

        taus = np.append(
            np.flip(-np.arange(rendering, taumax, rendering)),
            np.arange(0, taumax, rendering),
        )
    ...
    def _get_hours_and_labels(self, which, which_labels, start_at_zero=False):
        if which == "hourly":
            ...
            if which_labels == "roman_numerals":
                ...
                raise Exception(
                    "In combination with which='hourly', the labeltype must be 'roman_numerals', 'arabic_numerals' or 'no_labels'."
                )
            ...
            if which_labels == "roman_numerals":
                ...
                raise Exception(
                    "In combination with which='half-hourly', the labeltype must be 'roman_numerals', 'arabic_numerals' or 'no_labels'."
                )
            ...
            if which_labels == "roman_numerals":
                ...
                raise Exception(
                    "In combination with which='two-hourly', the labeltype must be 'roman_numerals', 'arabic_numerals' or 'no_labels'."
                )
            ...
            if which_labels == "no_labels":
                ...
                raise Exception(
                    "If a list is provided for which, which_labels must be either a list of the same length containing the labels or 'no_labels'. "
                )
        else:
            raise Exception(
                "which must be 'hourly', 'half-hourly', 'two-hourly' or a list with the desired hours"
            )
    ...
    def _add_apparent_solar_time(
        self, taus, labels, gnomon_type="nodus", style=None, labelstyle=None
    ):
        if self.nh:
        ...
        if gnomon_type == "nodus":
            for tau, label in zip(taus, labels):
                if np.all(
                    ...
                    self.plotter.plot_straight_line(
                        self._nodus(tau, decmax),
                        self._nodus(tau, decmin),
                        style=style,
                        label=label,
                        labelstyle=labelstyle,
                    )
            ...
            for tau, label in zip(taus, labels):
                ...
                self.plotter.plot_straight_line(
                    origin,
                    origin + length * np.array([np.sin(angle), np.cos(angle)]),
                    style=style,
                    label=label,
                    labelstyle=labelstyle,
                )
    ...
    def _add_mean_solar_time(
        ...
        if half_year == "spring":
            ts = np.flip(
                np.append(
                    np.arange(cm.find_solstice("winter"), 1, rendering),
                    np.arange(0, cm.find_solstice("summer"), rendering),
                )
            )
        elif half_year == "autumn":
            ts = np.arange(
                cm.find_solstice("summer"), cm.find_solstice("winter"), rendering
            )
        ...
        for tau, label in zip(taus, labels):
            ...
            for t in ts:
                tmp = self._nodus(
                    tau + cm.equation_of_time(t) * np.pi / 720, cm.declination(t)
                )
                X = np.append(X, tmp[0])
                Y = np.append(Y, tmp[1])
            self.plotter.plot_curve(
                X, Y, style=style, label=label, labelstyle=labelstyle
            )

    def add_mean_solar_time(
        self,
        which="hourly",
        half_year="spring",
        style=None,
    ...
    def add_babylonian_hours(
        self, which="hourly", style=None, which_labels="roman_numerals", labelstyle=None
    ):
        """
        ...
        if (self.phi > np.pi / 2 - const.ECLIPTIC / 180 * np.pi) or (
            ...
            raise Exception(
                "Babylonian hours are not supported for latitudes beyond the polar circles."
            )

        hours, labels = self._get_hours_and_labels(
            which, which_labels, start_at_zero=True
        )
        ...
        for hour, label in zip(hours, labels):
            ...
            if -maxtau_max < tau_summer < maxtau_max:
                if np.all(
                    np.isfinite(self._nodus(tau_summer, decl_summer_solstice))
                ) and np.all(
                    np.isfinite(self._nodus(tau_winter, decl_winter_solstice))
                ):
                    self.plotter.plot_straight_line(
                        self._nodus(tau_summer, decl_summer_solstice),
                        self._nodus(tau_winter, decl_winter_solstice),
                        style=style,
                        label=label,
                        labelstyle=labelstyle,
                    )
                else:
                    decmin, decmax = self._max_decs(
                        lambda x: self._nodus(
                            -bf.sunset(x, self.phi) + hour * np.pi / 12, x
                        )
                    )
                    taumin, taumax = (
                        -bf.sunset(decmin, self.phi) + hour * np.pi / 12,
                        -bf.sunset(decmax, self.phi) + hour * np.pi / 12,
                    )
                    self.plotter.plot_straight_line(
                        self._nodus(taumax, decmax),
                        self._nodus(taumin, decmin),
                        style=style,
                        label=label,
                        labelstyle=labelstyle,
                    )

    def add_italian_hours(
        self, which="hourly", style=None, which_labels="roman_numerals", labelstyle=None
    ):
        """
        ...
        for hour, label in zip(hours, labels):
            ...
            if -maxtau_max < tau_summer < maxtau_max:
                if np.all(
                    ...
                    decmin, decmax = self._max_decs(
                        lambda x: self._nodus(
                            bf.sunset(x, self.phi) - 2 * np.pi + hour * np.pi / 12, x
                        )
                    )
                    taumin, taumax = (
                        bf.sunset(decmin, self.phi) - 2 * np.pi + hour * np.pi / 12,
                        bf.sunset(decmax, self.phi) - 2 * np.pi + hour * np.pi / 12,
                    )
                    self.plotter.plot_straight_line(
                        self._nodus(taumax, decmax),
                        self._nodus(taumin, decmin),
    ...
    def add_temporal_hours(
        ...
        if (self.phi > np.pi / 2 - const.ECLIPTIC / 180 * np.pi) or (
            ...
            raise Exception(
                "Temporal hours are not supported for latitudes beyond the polar circles."
            )
        ...
        for hour, label in zip(hours, labels):
            ...
            if -maxtau_max < tau_summer < maxtau_max:
                if np.all(
                    ...
                    decmin, decmax = self._max_decs(
                        lambda x: self._nodus(
                            bf.sunset(x, self.phi) * (hour / 6 - 1), x
                        )
                    )
                    taumin, taumax = (
                        bf.sunset(decmin, self.phi) * (hour / 6 - 1),
                        bf.sunset(decmax, self.phi) * (hour / 6 - 1),
                    )
                    self.plotter.plot_straight_line(
                        self._nodus(taumax, decmax),
                        self._nodus(taumin, decmin),
