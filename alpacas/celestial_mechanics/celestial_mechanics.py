...
import numpy as np
from numpy import cos, sin, arccos, arctan, arctan2, sqrt, pi
...
def sun_coordinate(t, exact=True):
    ...
    """
    kt = kepler_trajectory(
        2 * pi * t
        - convert_true_to_mean_anomaly(
            const.LONGITUDE_OF_PERHELION / 180 * pi, const.ECCENTRICITY
        ),
        const.ECCENTRICITY,
        exact,
    )
    ...
    )  # vector of long half axis in equatorial coordinate system
    v2 = np.array(
        [-sin(beta), cos(alpha) * cos(beta), sin(alpha) * cos(beta)]
    )  # vector of short half axis in equatorial coordinate system
...
def declination_date(month, day, leap_year=False, exact=True):
    ...
    if leap_year:
        t = (
            dt.datetime(4, month, day)
            - dt.datetime(4, const.EQUINOX[0], const.EQUINOX[1])
        ).days / const.YEAR
    else:
        t = (
            dt.datetime(1, month, day)
            - dt.datetime(1, const.EQUINOX[0], const.EQUINOX[1])
        ).days / const.YEAR
...
def equation_of_time(t, eot_at_equinox=None, exact=True):
    ...
    if eot_at_equinox is None:
        offset = (
            ...
            - const.LONGITUDE_OF_PERHELION / 180 * pi
        ) / (2 * pi)
    else:
