...
class Plotter:
    ...
    def plot_point(self, pos, style=None):
        ...
        self.ax.plot(
            [pos[0]], [pos[1]], "o", markersize=style["ms"], color=style["color"]
        )

    def find_text_position_straight_line(
        self,
        A,
        B,
        side="left",
        ...
        if rotate:
            if rotation_type == "perpendicular":
                ...
                raise Exception(
                    "rotation_type must be 'perpendicular', 'parallel_up' or 'parallel_down'"
                )
        else:
            angle = 0
    ...
    def find_text_position_curve(
        ...
        dist = np.cumsum(
            np.append(
                np.array([0]),
                np.sqrt((np.roll(X, -1) - X) ** 2 + (np.roll(Y, -1) - Y) ** 2)[0:-1],
            )
        )
        ...
        if position_type == "relative":
            ind = max(
                min(np.argmin(np.abs(dist - dist[-1] * position)), np.size(X) - 2), 1
            )
        elif position_type == "absolute":
    ...
    def plot_straight_line(self, A, B, style=None, label=None, labelstyle=None):
        ...
        if (label is not None) and np.all(np.isfinite(np.append(A, B))):
            pos = self.find_text_position_straight_line(
                A,
                B,
                side=labelstyle["side"],
                position=labelstyle["position"],
                position_type=labelstyle["position_type"],
                tightness=labelstyle["tightness"],
    ...
    def plot_curve(self, X, Y, style=None, label=None, labelstyle=None):
        ...
        self.ax.plot(
            X, Y, lw=style["lw"], color=style["color"], linestyle=style["linestyle"]
        )

        if label is not None:
            pos = self.find_text_position_curve(
                X,
                Y,
                side=labelstyle["side"],
                position=labelstyle["position"],
